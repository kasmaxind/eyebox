...
export default function HomePage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (category === "Watched") {
      return videos.filter((v) => history.includes(v.id));
    }
    return videos.filter((v) => v.category === category || (category === "Live" && v.isLive));
  }, [category, history]);
  ...
  return (
    <div className="px-3 py-4 md:px-6 md:py-5">
      ...
      <section className="mb-8 overflow-hidden rounded-3xl border border-border/80 bg-gradient-to-br from-[#1a1220] via-bg-elevated to-[#0f1724] p-5 md:p-7 fade-in">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="max-w-xl">
            <p className="mb-2 inline-flex items-center gap-2 rounded-full bg-accent-soft px-3 py-1 text-xs font-semibold text-rose-200">
